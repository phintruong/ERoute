...
export interface FurnitureTemplate {
  label: string;
  relativeX: number;  // position within room unit
  relativeZ: number;
  width: number;      // meters
  depth: number;      // meters
  height: number;     // meters (used for block height in 3D)
...
export interface RoomTypeDefinition {
  ...
  label: string;
  description: string;
  unitWidth: number;   // meters per room unit
  unitDepth: number;   // meters per room unit
  floorColor: string;
  wallColor: string;
