...
export function runSimulation(
  ...
  for (const h of hospitals) {
    const snap = snapshots.find((s: any) => s.hospitalId === h._id.toString());
    loadMap[h._id.toString()] = snap?.occupancyPct ?? 70;
  }

  const patients: Array<{ lat: number; lng: number; severity: string; hospitalId: string }> = [];
  for (const h of hospitals) {
    const count = Math.round((loadMap[h._id.toString()] / 100) * h.erBeds * 2);
  ...
  for (const h of allHospitals) newCounts[h._id.toString()] = 0;

  for (const patient of patients) {
    const currentHospital = hospitals.find((h: any) => h._id.toString() === patient.hospitalId);
    ...
    const nearest = allHospitals
      .map(h => ({ id: h._id.toString(), dist: haversine(patient.lat, patient.lng, h.latitude, h.longitude) }))
